...
async function fetchImages(page = 1) {
  try {
    ...
    const response = await fetch(
      `http://localhost/modulo-imagenes/modulo-imagenes/backend/get_all_b64_2.php?page=${page}`
    );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function mostrarCartasEnSidebar() {
    ...
    fetch(`http://localhost/modulo-imagenes/modulo-imagenes/backend/get_all_b64_2.php?page=1`)
      ...
      .then(data => {
        ...
        for (let i = 1; i <= totalPages; i++) {
          let promise = fetch(`http://localhost/modulo-imagenes/modulo-imagenes/backend/get_all_b64_2.php?page=${i}`)
            .then(response => response.json())
            .then(pageData => {
              if (pageData.images) {
...
async function fetchRelatedImages(carta_front) {
  try {
    ...
    const response = await fetch(
      `http://localhost/modulo-imagenes/modulo-imagenes/backend/get_related_images.php?carta_front=${carta_front}`
    );
    const data = await response.json();
